...
class depthwiseconv_mix(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, padding):
        ...
        self.depth_conv = nn.Conv2d(in_ch, out_ch, kernel_size, padding, groups=in_ch)
        self.point_conv = nn.Conv2d(in_channels=out_ch,
                                    out_channels=out_ch,
                                    kernel_size=1,
                                    stride=1,
                                    padding=1,
...
class Fire(nn.Module):  # 新的网络架构Fire Module，通过减少参数来进行模型压缩
    ...
    def __init__(
            ...
            squeeze_planes: int,  # 输出通道  16  挤压层
            expand1x1_planes: int,  # 64
            expand3x3_planes: int,  # 64
    ) -> None:
        super(Fire, self).__init__()
        self.inplanes = inplanes
        self.squeeze = nn.Conv2d(inplanes, squeeze_planes, kernel_size=1)  # 56*56*16 挤压层
        self.squeeze_bn = nn.BatchNorm2d(squeeze_planes)  # 加入bn层 挤压层
        self.squeeze_activation = nn.ReLU(inplace=True)  # 挤压层
        self.expand1x1 = nn.Conv2d(squeeze_planes, expand1x1_planes,
                                   kernel_size=1)  # 56*56*64
        self.expand1x1_bn = nn.BatchNorm2d(expand1x1_planes)  # 加入bn层
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # torch.cat()将expand1x1_activation和 expand3x3_activation这两个维度相同的输出张量连接在一起
        return torch.cat([
    ...
    def __init__(
        ...
        if version == '1_0':
            self.features = nn.Sequential(  # self.features定义了主要的网络层
                ...
                nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),  # maxpool4
