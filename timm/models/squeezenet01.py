...
class Resblock_body(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.blocks_conv = nn.Sequential(
            Fire(96, 16, 64, 64, 0),
            Fire(128, 16, 64, 64, 1),  # 主程序squeezenet中位于前两个池化层中间的三个Fire模块添加到此处
...
class Resblock_body1(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.blocks_conv = nn.Sequential(
            ...
            Fire(384, 48, 192, 192, 1),  # 主程序squeezenet中位于后两个池化层中间的四个Fire模块添加到此处
...
class SqueezeNet(nn.Module):
    ...
    def __init__(
        ...
        if version == '1_0':
            self.features = nn.Sequential(  # self.features定义了主要的网络层
                ...
                # Fire(96, 16, 64, 64,0),
                # Fire(128, 16, 64, 64,1),
                # Fire(128, 32, 128, 128,0),   #  加了残差模块后，这三个Fire模块加到相应的残差模块。
                ...
                # Fire(384, 64, 256, 256,0),   #  加了残差模块后，这四个Fire模块加到相应的残差模块。
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # tensor张量
        x = self.features(x)
        x = self.classifier(x)
